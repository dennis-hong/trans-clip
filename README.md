...
![TransClip Screenshot](https://img.shields.io/badge/Platform-macOS-blue) ![Version](https://img.shields.io/badge/Version-0.1.21-green) ![License](https://img.shields.io/badge/License-MIT-yellow)

## ✨ 주요 기능

- 🔄 **즉시 번역** - 텍스트 선택 후 `Cmd+C` 두 번으로 바로 번역
- ✨ **윤문(Polish)** - 텍스트 선택 후 `Cmd+E` 두 번으로 문법/어조/명확성 개선
- 📋 **클립보드 히스토리** - `Cmd+Option+V`로 앱 창 표시 및 복사 기록 관리
...
- **접근성 권한** (단축키 감지 및 클립보드 모니터링용):
  ...
  - 재시작하지 않으면 `Cmd+C` 두 번, `Cmd+E` 두 번 등의 단축키가 동작하지 않습니다
...
1. **다듬을 텍스트를 선택**
2. **`Cmd+E`를 빠르게 두 번** 누르기 (500ms 이내)
...
1. **`Cmd+Option+V`** 또는 메뉴바 아이콘 클릭으로 앱 창 표시
2. 최근 복사한 항목들이 표시됩니다
3. `Escape`로 창을 숨기고, 다시 `Cmd+Option+V`로 불러올 수 있습니다
4. 숫자 키로 빠르게 선택:
   - `1-9`: 해당 항목을 클립보드에 복사
   - `Shift+숫자`: 해당 항목 번역
   - `Ctrl+숫자`: 해당 항목 윤문
...
| 윤문 팝업 | `Cmd+E` 두 번 (500ms 이내) |
| 앱 창 표시 / 클립보드 히스토리 | `Cmd+Option+V` |
